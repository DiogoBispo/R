...
ui <- fluidPage(
    ...
    fluidRow(
        column(4,sliderInput("sexatas", "Gosto por Exatas", min=5, max = 90, sep = 10, value = 40)),
        column(4,sliderInput("sinter", "Relacionamento Interpessoal", min = 30, max = 85, sep = 5, value = 50)),
        column(4,sliderInput("scodigo", "Gosto por escrever codigo",min = 10, max = 95,step = 5,value = 40))
        
    ),
    fluidRow(
        column(4,sliderInput("slider", "Perfil de Lideranca", min = 30, max = 95, step = 5, value = 50)),
        column(4,sliderInput("sestudar", "Gosta de Estudar", min = 20, max = 90, step = 10, value = 40)),
        column(4,sliderInput("scomunica", "Habilidade de comunicacao", min = 40, max = 75, step = 5, value = 50))
        
    ),
    fluidRow(
        column(6,h1("Sistema:"), plotOutput("Grafsistema")),
        column(6,actionButton("Processar", "Processar"),helpText("A linha vermelha mostra a tua Aderencia a Profissao de CienTista de Dados"),
...
server <- function(input, output) {
    ...
    observeEvent(input$Processar,{
      inferencia <- fuzzy_inference(sistema, list(Gosta_exatas = input$sexatas, Rel_interpess = input$sinter, Escre_codigo = input$scodigo, 
                                                 Perfil_lider = input$slider,  Gosta_Estudar = input$sestudar, Habilidade_comunicao = input$scomunica))
